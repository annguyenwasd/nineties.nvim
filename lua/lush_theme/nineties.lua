...
local theme = lush(function(injected_functions)
	...
	return {
		...
		DiffAdd({ fg = green.darken(10) }), -- Diff mode: Added line |diff.txt|
		DiffChange({ fg = green }), -- Diff mode: Changed line |diff.txt|
		DiffDelete({ fg = red.darken(30) }), -- Diff mode: Deleted line |diff.txt|
		-- DiffText       {bg=black,fg=red }, -- Diff mode: Changed text within a changed line |diff.txt|
		-- EndOfBuffer    { }, -- Filler lines (~) after the end of the buffer. By default, this is highlighted like |hl-NonText|.
		TermCursor({ bg = white }), -- Cursor in a focused terminal
		-- TermCursorNC   { }, -- Cursor in an unfocused terminal
		-- ErrorMsg       { }, -- Error messages on the command line
		-- VertSplit      { }, -- Column separating vertically split windows
		Folded({ fg = white.da(20) }), -- Line used for closed folds
		...
		StatusLine({ bg = black, fg = white }), -- Status line of current window
		-- StatusLineNC   {}, -- Status lines of not-current windows. Note: If this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
		-- TabLine        { }, -- Tab pages line, not active tab page label
		-- TabLineFill    { }, -- Tab pages line, where there are no labels
		-- TabLineSel     { }, -- Tab pages line, active tab page label
		Title({}), -- Titles for output from ":set all", ":autocmd" etc.
		Visual({ bg = black.li(10) }), -- Visual mode selection
		...
		Type({ gui = "bold" }), -- (*) int, long, char, etc.
		...
		Todo({ gui = "bold", fg = Normal.fg.darken(35) }), -- Anything that needs extra attention; mostly the keywords TODO FIXME and XXX
		...
		sym("@boolean")({}), -- Boolean
		sym("@float")({}), -- Float
		sym("@function")({ gui = "bold" }), -- Function
		sym("@function.builtin")({ gui = "bold" }), -- Special
		sym("@function.macro")({ gui = "bold" }), -- Macro
